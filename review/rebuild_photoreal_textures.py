#!/usr/bin/env python3
"""Rebuild museum texture files from documented source assets.
...
extensions, caches source downloads, writes structured provenance, and emits 3x3
...
def write_manifests() -> tuple[Path, Path]:
    ...
    texture_path = REVIEW_DIR / "photoreal_texture_manifest.json"
    source_path = REVIEW_DIR / "texture_source_manifest.json"
    texture_path.write_text(json.dumps({
        "schemaVersion": "museum-legacy-textures/1.0",
        "generator": "review/rebuild_photoreal_textures.py",
        "warning": "Legacy rebuild manifest only. Room-level historical approval and the central material manifest remain authoritative.",
        "textures": rows,
    }, indent=2) + "\n")
    source_path.write_text(json.dumps({
        "schemaVersion": "museum-texture-sources/1.0",
        "licensePolicy": "ambientCG sources are CC0; Commons sources retain the listed per-file license. A source image does not itself authorize cultural reuse.",
        "sources": [
            {
                "key": key,
                "label": source.label,
                ...
                "cachedFile": str(source.path.relative_to(ROOT)),
            }
            for key, source in sorted(SOURCES.items())
        ],
    }, indent=2) + "\n")
    return texture_path, source_path
...
def main() -> int:
    ...
    build_contact_sheet()
    texture_manifest, source_manifest = write_manifests()
    print(f"Processed {len(LEDGER)} texture files.")
    print(f"Review: {texture_manifest}")
