...
- architecture_match: 4 — pier rhythm, stepped portal masses, ceiling beams, and benches match the concept layout; carved depth is shallower than concept (bands sit flat, no recessed relief bays).
- decor_props_match: 3 — benches present and carved header panels exist over the entrance, but no sculptural focal piece/altar on the doorway axis and the hero relief is reduced to a mirrored fragment pair.
- entrance_signage: 2 — no MESOAMERICA title sign is visible in any of the four threshold captures.
...
## Hero element
Partially. The Mitla-style greca fret band running the full corridor plus the stepped portal with carved header panels is recognizable as Mesoamerica at a glance, but the intended hero — the deity-mask panel over the ceremonial portal — is currently a mirrored, cropped fragment rather than a readable mask. Strongest candidate from the concept sheet to fix/promote: the deity-mask lintel cartouche over the stepped entrance.

## Signage
Not visible. None of entrance_out, entrance_in, exit_out, or exit_in shows a MESOAMERICA title sign (entrance_out shows only the black header with carved panels above the opening; entrance_in looks out at the hub where a neighboring "…ANIA" sign is the only legible text). No quotable room-title text at either threshold.
