...
- architecture_match: 3 — corridor with beam ceiling, recessed relief bays, and a columned portal are all present; portal columns are smooth flat cylinders with no fluting and no bull-protome capitals, and the ceiling beams are far sparser than the concept's dense linear beams.
- decor_props_match: 2 — guard relief panels and rosette dado band exist, but the faravahar/winged-disk is nowhere visible, there are no bronze accent lamps or bench, and the signature guard decor is ruined by inversion.
- entrance_signage: 1 — no room title sign visible or legible in either threshold view; only artwork placards are present.
...
## Hero element
No. The intended hero — the Apadana portal crowned by a gold faravahar with bull-capital columns — is absent (portal top is bare, columns plain), and the fallback hero, the processional guard relief panels, is installed upside-down so it cannot read as Achaemenid at a glance. Strongest candidate from the concept art: the gold faravahar over the entrance portal, backed by right-side-up guard reliefs flanking the doorway.

## Signage
Not visible. Neither entrance_out.jpg nor entrance_in.jpg shows any room title sign at the threshold — no "Persia", "Hallway 16", or era text. The only readable text anywhere in the views is artwork placards (e.g. "Apadana Reliefs, Persepolis" in floor.jpg, "The Great Isaiah Scroll — loading image" in exit_out.jpg).
