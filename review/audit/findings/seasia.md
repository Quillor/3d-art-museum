...
- architecture_match: 3 — coursed block walls, timber ceiling beams, and a stepped threshold portal flanked by relief panels are present; corbelled ceiling depth and lintel relief are missing, and the ringed drum columns read generic rather than Khmer colonette.
- decor_props_match: 2 — apsara panels alternate correctly between artworks per spec, but every one is inverted, and the lintel relief field is absent from all doorways.
- entrance_signage: 1 — no "Southeast Asia" title sign visible at the threshold in either entrance view; the only legible sign is the far room's "JAPAN".
...
## Hero element
Not yet. The intended hero — uplit carved apsara/devata relief panels flanking the portal and alternating along the walls, matching the concept's bas-relief gallery and Temple Doorway Portal — is now present and correctly placed, but the upside-down texture destroys the read. Flipping the panel texture right-side-up (and adding the lintel relief over the portal) would give the room its instantly-recognizable signature; no alternative candidate is needed.

## Signage
No. Neither entrance_out.jpg nor entrance_in.jpg shows a legible seasia room title at the threshold. The only readable text in the entrance sightline is "JAPAN" (with an illegible smaller line beneath) above the far portal, which belongs to the next room.
