...
- architecture_match: 4 — fluted columns, coffered ceiling, wall aediculae, and marble entrance portal all present; columns end in plain blocks with no entablature beam connecting them.
- decor_props_match: 3 — Greek-key frieze, anthemion band, wave-scroll band, and wall lamps present; the mosaic "carpet" doesn't read as tesserae and the aedicula niches are empty.
- entrance_signage: 2 — no legible room title at the threshold; only an illegible dark oval floor medallion with gold text in the approach rotunda (entrance_in) and an unreadable black board over the far exit arch (entrance_out).
...
## Hero element
Yes — the polychrome coffered ceiling over a fluted colonnade, capped by the red Greek-key meander frieze, gives an instant Greco-Roman read matching the concept's signature. Its impact is diluted by the flat pastel carpet below; making the mosaic carpet read as real tesserae would complete the concept's second hero (the "mosaic central carpet").

## Signage
Not legible. entrance_out shows no title over the entry portal (only an unreadable black board above the distant exit arch), and entrance_in shows only a dark oval floor medallion with illegible gold lettering in the preceding rotunda. No readable text to quote.
